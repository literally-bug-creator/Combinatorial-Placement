...
class TestReplacements(unittest.TestCase):
  def test_GetReplacementsCount(self):
    ...
    self.assertEqual(GetPlacementsCount(64, 63), 126886932185884164103433389335161480802865516174545192198801894375214704230400000000000000)
    ...
    self.assertEqual(GetPlacementsCount(99, 55), 351078559200149679413117488175363602453135331275499185304858030453069232236379811746938880000000000000)
    ...
    self.assertEqual(GetPlacementsCount(800, 100), )
    self.assertEqual(GetPlacementsCount(150, 40), 3597292614445561520158671789370178969359700771419675347809280434032449945600000000000)
    self.assertEqual(GetPlacementsCount(44, 43), 2658271574788448768043625811014615890319638528000000000)
    self.assertEqual(GetPlacementsCount(55, 55), 12696403353658275925965100847566516959580321051449436762275840000000000000)
  ...
  def test_GetNextPlacementInLexicographicOrder(self):
    ...
    self.assertEqual(GetNextPlacementInLexicographicOrder(12, 10, [4, 3, 2, 1, 5, 6, 7, 8, 9, 10]), [4, 3, 2, 1, 5, 6, 7, 8, 9, 11])
    self.assertEqual(GetNextPlacementInLexicographicOrder(12, 10, [4, 3, 2, 1, 5, 6, 7, 8, 9, 11]), [4, 3, 2, 1, 5, 6, 7, 8, 9, 12])
    self.assertEqual(GetNextPlacementInLexicographicOrder(11, 10, [4, 3, 2, 1, 5, 6, 7, 8, 9, 10]), [4, 3, 2, 1, 5, 6, 7, 8, 9, 11])
    self.assertEqual(GetNextPlacementInLexicographicOrder(11, 10, [4, 3, 2, 1, 5, 6, 7, 8, 9, 10]), [4, 3, 2, 1, 5, 6, 7, 8, 9, 11])
